...
==Input:==
ano need to insert input from the user. 

==Output:==

will output the setting of a new state in this way : allocating the state (number of state):
from cur_state -> with this label -> to_state.
Will output the setting of the fail state to a state that the id is not 0: 
fail(cur_state) = to_state;
